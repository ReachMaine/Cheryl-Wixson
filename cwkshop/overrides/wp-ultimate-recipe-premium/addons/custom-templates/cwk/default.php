...
?>
<div class="wpurp-container layout-default" itemscope itemtype="http://schema.org/Recipe">
    ...
    <?php
    $thumb = wp_get_attachment_image_src( get_post_thumbnail_id($recipe_post->ID), 'recipe-med' );
    $thumb_url = $thumb['0'];

	if(!is_null($thumb_url) && (get_post_type() == "recipe") || get_post_meta( get_the_ID(), "show_recipe_image", true )) { // zig
            ...
            <ul class="recipe-header-name">
                <li class="recipe-information-name">
                    <span class="recipe-rating">
                        ...
                        if($this->is_premium_addon_active('user-ratings') && $this->option('user_ratings_enable', 'everyone') != 'disabled') {
                            include($this->premiumDir . '/addons/user-ratings/helper/public_rating.php');
                        } else {
				...
				<?php if (get_post_type() == "recipe") { // zig ?>
                     <li class="recipe-information-category">
                        <?php the_category('' ); ?><br />
                        <?php the_tags(''); ?>
                    </li>
                <?php } ?>
    ...
    <!-- <div class="wpurp-clear"></div> -->
     <div class="recipe-ingredients-servings">
    <?php if($this->option('recipe_adjustable_servings', '1') == '1') { ?>
    <div><?php _e( 'Servings', $this->pluginName ); ?>: <input type="number" class="advanced-adjust-recipe-servings" data-original="<?php echo $recipe['recipe_servings_normalized'][0]; ?>" value="<?php echo $recipe['recipe_servings_normalized'][0]; ?>" /> <?php echo $recipe['recipe_servings_type'][0]; ?></div>
    <?php } ?>
    <?php if ($this->option('recipe_adjustable_units', '1') == '1') { ?>
        <div><?php _e( 'Units', $this->pluginName ); ?>:
            <select onchange="RecipeUnitConversion.recalculate(this)" class="adjust-recipe-unit">
                <?php
                //TODO Static function
                include_once($this->pluginDir . '/helper/units.php');
                $unit_helper = new WPURP_Helper_Units();

                $systems = $unit_helper->get_active_systems();

                foreach($systems as $i => $system) {
                    echo '<option value="'.$i.'">'.$system['name'].'</option>';
                }
                ?>
            </select>
        </div>
    <?php } ?>
    </div> <!-- end recipe-ingredients-servings -->
    <?php
    ...
    <ul class="recipe-ingredients" data-servings="<?php echo $recipe['recipe_servings_normalized'][0]; ?>">
        ...
        foreach($ingredients as $ingredient) {
            ...
            $out .= '<span class="recipe-ingredient-quantity-unit"><span class="recipe-ingredient-quantity" data-normalized="'.$ingredient['amount_normalized'].'" data-original="'.$ingredient['amount'].'">'.$ingredient['amount'].'</span> <span class="recipe-ingredient-unit" data-original="'.$ingredient['unit'].'">'.$ingredient['unit'].'</span></span>';
    ...
    <ol class="recipe-instructions">
        ...
        foreach($instructions as $instruction) {
            ...
            if($instruction['image'] != '') {
                $thumb = wp_get_attachment_image_src( $instruction['image'], 'recipe-med' );
                $thumb_url = $thumb['0'];
