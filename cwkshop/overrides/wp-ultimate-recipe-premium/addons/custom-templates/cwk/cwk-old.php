...
<div class="wpurp-container layout-default" itemscope itemtype="http://schema.org/Recipe">
    ...
    if(!is_null($thumb_url) && (get_post_type() == "recipe") { // zig
    ...
    <div><?php _e( 'Servings', $this->pluginName ); ?>: <input type="number" class="advanced-adjust-recipe-servings" data-original="<?php echo $recipe['recipe_servings'][0]; ?>" value="<?php echo $recipe['recipe_servings'][0]; ?>" /> <?php echo $recipe['recipe_servings_type'][0]; ?></div>
    <?php } ?>
    <?php if ($this->option('recipe_adjustable_units', '1') == '1') { //TODO Dynamic? ?>
    <div><?php _e( 'Units', $this->pluginName ); ?>:
    <select onchange="RecipeUnitConversion.recalculate(this)" data-ingredientscalcid="ingredientsCalc1" class="adjust-recipe-unit" name="ingredientsType">
        <option value="metric"><?php _e( 'Metric', $this->pluginName ); ?></option>
        <option value="imperial"><?php _e( 'Imperial', $this->pluginName ); ?></option>
    </select>
    </div>
    <?php } ?>
    </div>
